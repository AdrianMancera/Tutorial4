...
#        self.setupSimulationButtons(win)
    ...
    def animatePolicy(self):
        ...
        if not 'animatePolicyBox' in dir(self):
            ...
            self.canvas.create_text(x-60,y-old_div(length,4),text='Blue = kickLeft')
            self.canvas.create_text(x-60,y-old_div(length,4)+20,text='Red = kickRight')
            self.canvas.create_text(x-60,y-old_div(length,4)+40,text='White = doNothing')
