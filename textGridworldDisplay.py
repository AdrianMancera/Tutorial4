...
def indent(rows, hasHeader=False, headerChar='-', delim=' | ', justify='left',
    ...
    def rowWrapper(row):
        ...
        lists = [list(x) for x in list(itertools.zip_longest(*newRows))]
        return [[substr or '' for substr in item] for item in lists]
        #return [[substr or '' for substr in item] for item in list(*newRows)]
    # break each logical row into one or more physical ones
    logicalRows = [rowWrapper(row) for row in rows]
    # columns of physical rows
    columns = [tuple(x) for x in list(itertools.zip_longest(*reduce(operator.add,logicalRows)))]    
